...
const (
	minTotalReviewsForExtraReviews = 100
)
...
func NewPlaceJob(parentID, langCode, regionCode, u string, extractEmail, extraExtraReviews bool, opts ...PlaceJobOptions) *PlaceJob {
	const (
	...
	job := PlaceJob{
		Job: scrapemate.Job{
			...
			Method:     "GET",
			URL:        u,
			URLParams:  map[string]string{"hl": langCode, "gl": regionCode},
...
func (j *PlaceJob) BrowserActions(ctx context.Context, page scrapemate.BrowserPage) scrapemate.Response {
	...
	if j.ExtractExtraReviews {
		entry, ok := j.extraReviewCandidate(raw)
		if ok {
...
func (j *PlaceJob) extraReviewCandidate(data []byte) (Entry, bool) {
	entry, err := EntryFromJSON(data)
	if err != nil {
		return Entry{}, false
	}
	if entry.ReviewCount <= minTotalReviewsForExtraReviews {
		return Entry{}, false
	}
