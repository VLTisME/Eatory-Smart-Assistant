...
func NewGmapJob(
	id, langCode, regionCode, query string,
	maxDepth int,
	extractEmail bool,
...
func (j *GmapJob) Process(ctx context.Context, resp *scrapemate.Response) (any, []scrapemate.IJob, error) {
	...
	if strings.Contains(resp.URL, "/maps/place/") {
		...
		placeJob := NewPlaceJob(j.ID, j.LangCode, j.RegionCode, resp.URL, j.ExtractEmail, j.ExtractExtraReviews, jopts...)
		...
		doc.Find(`div[role=feed] div[jsaction]>a`).Each(func(_ int, s *goquery.Selection) {
			if href := s.AttrOr("href", ""); href != "" {
				...
				nextJob := NewPlaceJob(j.ID, j.LangCode, j.RegionCode, href, j.ExtractEmail, j.ExtractExtraReviews, jopts...)
