...
const (
	...
	maxReviewPages     = maxReviewsPerPlace / reviewPageSize
)
...
func (f *fetcher) fetch(ctx context.Context) (FetchReviewsResponse, error) {
	...
	reviewURL, err := f.generateURL(f.params.mapURL, "", reviewPageSize, requestIDForSession)
	if err != nil {
	...
	if f.params.page != nil {
		...
		log.Printf("Browser-based RPC fetch failed: %v", err)
	}

	return FetchReviewsResponse{}, errors.New("browser-based RPC fetch failed")
}
...
func (f *fetcher) fetchWithBrowser(_ context.Context, initialURL, requestID string) (FetchReviewsResponse, error) {
	...
	}`, initialURL, acceptLanguage(f.params.langCode))
	...
	for nextPageToken != "" && len(ans.pages) < maxReviewPages {
		nextURL, err := f.generateURL(f.params.mapURL, nextPageToken, reviewPageSize, requestID)
		if err != nil {
			break
		}
		...
		}`, nextURL, acceptLanguage(f.params.langCode))
...
func (f *fetcher) generateURL(mapURL, pageToken string, pageSize int, requestID string) (string, error) {
	...
	fullURL := fmt.Sprintf(
		"https://www.google.com/maps/rpc/listugcposts?authuser=0&hl=%s&gl=%s&pb=%s",
		url.QueryEscape(defaultString(f.params.langCode, "vi")),
		url.QueryEscape(defaultString(f.params.regionCode, "VN")),
...
func acceptLanguage(langCode string) string {
	langCode = defaultString(langCode, "vi")
	if strings.EqualFold(langCode, "vi") {
		return "vi-VN,vi;q=0.9,en;q=0.5"
	}

	return fmt.Sprintf("%s,%s;q=0.9,en;q=0.5", langCode, langCode)
}
...
func extractReviewsFromPage(ctx context.Context, page scrapemate.BrowserPage) ([]DOMReview, error) {
	...
	clickedReviews, _ := page.Eval(`() => {
		try {
			...
			for (const btn of reviewsButtons) {
				...
				if (text.includes('review') || label.includes('review') || text.includes('đánh giá') || label.includes('đánh giá')) {
					btn.click();
			...
			for (const tab of tabs) {
				...
				if (label.includes('review') || label.includes('đánh giá')) {
					tab.click();
					return 'reviews_tab';
				}
			}
			...
			for (const link of allLinks) {
				...
				if (text.includes('all review') || text.includes('see review') || text.includes('more review') || text.includes('tất cả bài đánh giá') || text.includes('xem bài đánh giá') || text.includes('thêm bài đánh giá')) {
					link.click();
	...
	for attempt := 0; attempt < maxScrollAttempts; attempt++ {
		...
		if currentCount >= maxReviewsPerPlace {
			break
		}
	...
	if len(reviews) > maxReviewsPerPlace {
		reviews = reviews[:maxReviewsPerPlace]
	}
